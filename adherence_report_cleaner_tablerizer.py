...
def process_employee_data(input_file, output_file=None):
    ...
    # Define output headers without Employee Group
    output_headers = [
        'Employee Name', 'Start', 'Stop', 'Duration',
    ...
    for index, row in df.iterrows():
        ...
        if any('/' in x for x in row_data) and len(row_data) >= 7:
            try:
                # Extract data fields (excluding Employee Group)
                record = {
    ...
    if records:
        ...
        # Reorder columns to have Employee Name first
        column_order = ['Employee Name'] + [col for col in output_df.columns if col != 'Employee Name']
...
def main():
    parser = argparse.ArgumentParser(description='Process employee time tracking data without Employee Group.')
