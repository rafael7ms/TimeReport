...
def apply_number_formatting(worksheet, start_row, end_row, col):
    """Apply number formatting to specified column"""
...
def process_time_report(input_file, output_file):
    ...
    try:
        ...
        filtered_df = df[df['Actual State'].isin(categories)].copy()
        logger.info(f"Filtered to {len(filtered_df)} records in specified categories")
        
        # Extract date from Start time
        filtered_df['Date'] = pd.to_datetime(filtered_df['Start']).dt.date
        ...
        agent_summary = filtered_df.groupby([
            ...
            'Date',
            'Actual State'
        ])['Total Hours'].sum().reset_index()
        
        agent_summary['Total Hours'] = agent_summary['Total Hours'].round(5)
        ...
        total_row = pd.DataFrame({
            ...
            'Date': [''],
            'Actual State': [''],
            'Total Hours': [agent_summary['Total Hours'].sum()]
        })
        agent_summary = pd.concat([agent_summary, total_row], ignore_index=True)
        ...
        # Agent Summary Sheet
        ws_agent = wb.active
        ws_agent.title = "Agent Summary"
        ws_agent.cell(row=1, column=1, value="Agent Summary").font = Font(size=14, bold=True)
        
        # Write agent data
        start_row = 3
        for r_idx, row in enumerate(dataframe_to_rows(agent_summary, index=False, header=True), start_row):
            for c_idx, value in enumerate(row, 1):
                ws_agent.cell(row=r_idx, column=c_idx, value=value)
        
        # Apply formatting
        end_row = start_row + len(agent_summary)
        apply_table_formatting(ws_agent, start_row, 1, end_row - 1, 5, "Agent_Table", "4472C4")
        apply_number_formatting(ws_agent, start_row + 1, end_row, 5)
        add_total_row_formatting(ws_agent, end_row, 5)
        ...
        apply_number_formatting(ws_cat, start_row_cat + 1, cat_end_row, 5)
